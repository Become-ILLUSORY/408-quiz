...
def pick_questions(db, counts, years, seed, rng_override=None):
...
def render_q(q, qnum, show_answer):
    ...
    stem = q["stem_html"] or html.escape(q["stem_text"])

    if q["options_in_svg"]:
        opts_html = '<div class="opts-in-svg">（选项为图中 A/B/C/D 所示）</div>'
    else:
        opts = []
        for i, o in enumerate(q["options"]):
            letter = o["label"] or "ABCD"[i]
            otext = o["html"] or html.escape(o["text"])
            opts.append(
                f'<label class="opt"><input type="radio" name="q{qnum}" value="{letter}">'
    ...
    if show_answer and q["answer"]:
        exp = q["explanation_html"] or html.escape(q["explanation_text"])
        ans_html = (
            f'<div class="q-answer" hidden data-answer="{html.escape(q["answer"])}">'
...
.opts-in-svg {{ color: #64748b; font-size: 13px; padding: 8px 0; }}
...
@media print {{
  ...
  .controls, .result-bar {{ display: none !important; }}
...
function score() {
  ...
  document.querySelectorAll('.q').forEach(function(q) {
    ...
    if (chosen && chosen.value === ans.dataset.answer) right += 2;
  });
...
function markQuestion(q, reveal) {
  ...
  ans.hidden = false;
  if (!reveal) return;
  var correct = ans.dataset.answer;
...
def main():
    ...
    picked = pick_questions(db, args.counts, years, seed)
    ...
    print(f"页面大小: {os.path.getsize(out)/1024:.0f} KB")
    if args.open:
        try:
            import subprocess
            subprocess.Popen(["minis-open", out])
        except Exception:
            pass
